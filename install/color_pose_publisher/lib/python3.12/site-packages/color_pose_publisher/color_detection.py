...
def detect_single_closest_cube(roi_frame, pixel_size_mm, offset_x, offset_y,color_priorities={'Red':1,'Blue':2,'Green':3}):
    """Detect all cubes, return pose of the highest priority cube (color then proximity)."""
    hsv = cv2.cvtColor(roi_frame, cv2.COLOR_BGR2HSV)

    # Apply CLAHE on the V channel for consistent lighting
    h, s, v = cv2.split(hsv)
    clahe = cv2.createCLAHE(clipLimit=2.0, tileGridSize=(8, 8))
    v = clahe.apply(v)
    hsv = cv2.merge([h, s, v])
    ...
    # Color priority: lower number = higher priority
    ...
    for color_name, ranges in colors.items():
        ...
        for cnt in contours:
            ...
            approx = cv2.approxPolyDP(cnt, 0.04 * peri, True)
            if len(approx) != 4:
                continue  # We want roughly square shapes

            x, y, w, h = cv2.boundingRect(cnt)
            aspect_ratio = w / float(h)
            if not (0.85 <= aspect_ratio <= 1.15):
                continue  # Must be roughly square
            ...
            if pose[0] is not None:
                x_mm, y_mm, z_mm, theta_x, theta_y, theta_z = pose
                detected_poses.append((color_priorities[color_name],  x_mm, y_mm, theta_z, color_name))

    if not detected_poses:
        return None  # No cubes detected in this frame

    # Sort by priority (lowest first), then by Y (lowest first)
    detected_poses.sort()

    # Pick the best one (highest priority + closest)
    _, x_mm, y_mm, theta_z, color_name = detected_poses[0]

    print(theta_z)

    return (x_mm, y_mm, theta_z, color_name)
