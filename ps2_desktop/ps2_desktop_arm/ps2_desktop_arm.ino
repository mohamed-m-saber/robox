...
const byte zeroPos[NUM_JOINTS]  = {95.555454545, 90.5745, 86.56364, 90, 90, 30};
...
float applyCalibration(byte i, float val, bool forward) {
  ...
  if (i == 1) return forward ? val + 0.5745 : val - 4.639;
  if (i == 2) return forward ? val - 3.43636 : val + 5.6363;
  if (i == 7) return forward ? val - 0.3 : val + 0.3;
  return val;
}
...
void parseSerial() {
  ...
  for (byte j = 0; j < NUM_JOINTS; j++) {
    ...
    float angle = map(originalInputValues[j], 0, 100, minAngle[j], maxAngle[j]);
    angle = applyCalibration(j, angle, true);
    moveToTarget(j, angle);
  }
}

void moveToTarget(byte joint, float angle) {
  targetPos[joint] = constrain(angle, minAngle[joint], maxAngle[joint]);
}
