...
#             'red': {'x': 0.195, 'y': 0.125},
#             'green': {'x': 0.195, 'y': 0.125},
#             'blue': {'x': 0.195, 'y': 0.125},
#         }
...
class PickAndPlaceServer(Node):
    ...
    async def execute_callback(self, goal_handle):
        ...
        if self.counter == 0:
            self.stack_pose = {'x': x, 'y': y, 'rotation': z}
            send_feedback(f'Setting stack pose for first {color} cube at ({x:.3f}, {y:.3f})')
            self.counter += 1
            goal_handle.succeed()
            return PickAndPlace.Result(success=True, message=f"Stack pose set from first {color.capitalize()} cube.")

        if self.stack_pose is None:
            msg = "No stack pose defined"
            self.get_logger().warn(msg)
            goal_handle.abort()
            return PickAndPlace.Result(success=False, message=msg)
        ...
        home_wrist_angle = self.compute_wrist_angle(0.0, 0.0, 0.0)
        pick_wrist_angle = self.compute_wrist_angle(x, y, z)
        place_wrist_angle = self.compute_wrist_angle(self.stack_pose['x'], self.stack_pose['y'], self.stack_pose['rotation'])
        ...
        def plan_and_wait(x, y, z, wrist_angle):
            ...
            plan_request = MotionPlanRequest()
            plan_request.group_name = 'arm'
            plan_request.allowed_planning_time = 0.3
            plan_request.num_planning_attempts = 2000
            plan_request.goal_constraints.append(self.create_pose_constraint(pose_target))
            plan_request.max_velocity_scaling_factor = 1.0
            plan_request.max_acceleration_scaling_factor = 1.0
        ...
        try:
            ...
            send_feedback("Approaching pick position")
            plan_and_wait(x, y, self.approach_z - 0.1, pick_wrist_angle)
            ...
            if (y <= 0.125 and self.stack_pose['y'] > 0):
                send_feedback("Centering Cube")
                plan_and_wait(0.105, 0.125, self.approach_z + 0.05, pick_wrist_angle)

            elif (y > 0.125 and self.stack_pose['y'] < 0):
                send_feedback("Centering Cube")
                plan_and_wait(0.105, 0.125, self.approach_z + 0.05, pick_wrist_angle)

            send_feedback("Moving to stack pose")
            plan_and_wait(self.stack_pose['x'], self.stack_pose['y'], self.approach_z, place_wrist_angle)

            # i=self.approach_z
            # while (i>=self.place_z + (self.counter * 0.03)):
            send_feedback("Lowering to stack")
            plan_and_wait(self.stack_pose['x'], self.stack_pose['y'], self.place_z + (self.counter * 0.03), place_wrist_angle)
                # i=i-0.01
