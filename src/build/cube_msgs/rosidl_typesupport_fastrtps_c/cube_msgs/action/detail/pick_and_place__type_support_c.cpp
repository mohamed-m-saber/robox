...
#include "rosidl_runtime_c/string.h"  // color, qr
#include "rosidl_runtime_c/string_functions.h"  // color, qr
  ...
  eprosima::fastcdr::Cdr & cdr)
{
  ...
  // Field name: qr
  {
    const rosidl_runtime_c__String * str = &ros_message->qr;
    if (str->capacity == 0 || str->capacity <= str->size) {
      fprintf(stderr, "string capacity not greater than size\n");
      return false;
    }
  ...
  cube_msgs__action__PickAndPlace_Goal * ros_message)
{
  ...
  // Field name: qr
  {
    std::string tmp;
    cdr >> tmp;
    if (!ros_message->qr.data) {
      rosidl_runtime_c__String__init(&ros_message->qr);
    }
    bool succeeded = rosidl_runtime_c__String__assign(
      &ros_message->qr,
      tmp.c_str());
    if (!succeeded) {
      fprintf(stderr, "failed to assign string into field 'qr'\n");
      return false;
    }
  }
