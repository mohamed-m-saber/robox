...
class PickAndPlaceServer(Node):
    def __init__(self):
        ...
        self.predefined_xyz_poses = {
            ...
            'bina': {'x': 0.04201, 'y': 0.407},
            'binb': {'x': 0.04201, 'y': 0.407},
        }
    ...
    async def execute_callback(self, goal_handle):
        qr = goal_handle.request.qr.lower()
        if qr not in self.predefined_xyz_poses:
            msg = f"No predefined bin pose for {qr}"
        ...
        send_feedback(f'Planning pick for {qr} cube at ({x:.3f}, {y:.3f})')
        ...
        try:
            ...
            send_feedback("Lowering to bin")
            plan_and_wait(bin_pose['x'], bin_pose['y'], self.place_z, place_wrist_angle)
        ...
        goal_handle.succeed()
        return PickAndPlace.Result(success=True, message=f"{qr.capitalize()} cube placed successfully.")
